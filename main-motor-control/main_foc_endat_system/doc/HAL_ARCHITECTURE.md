...
|       |    \- configure EPWM7 as the EnDat producer scheduler
|       +- HAL_setupCMPSS()
...
|       +- register endatProducerISR on EPWM7 INT
...
|       +- clear EPWM7 interrupt flag
|       +- enable EPWM1 INT in PIE group 3
|       +- enable EPWM7 INT in PIE group 3
|       \- enable CPU INT3
|
\-[17] EINT / ERTM
```
...
| `HAL_setupMotorPWMs()` | `HAL_setMotorParams()` | Configure EPWM1/2/3 for motor drive and EPWM7 for EnDat scheduling |
| `HAL_setupEndatProducerPWM()` | `HAL_setupMotorPWMs()` | Configure EPWM7 as an internal `40 kHz` EnDat scheduler |
...
| EPWM7 | EnDat runtime scheduler | Internal `40 kHz` producer time base |
...
| EPWM7 INT | Group 3 / INT7 | `endatProducerISR()` |
...
| `endatProducerISR()` | EPWM7 INT | Independent EnDat runtime producer scheduler |
...
Because EPWM7 is now reserved for the EnDat producer scheduler, `DACOUT_EN` is compile-time blocked for this build configuration.
...
`HAL_setupEndatProducerPWM()` configures EPWM7 as an internal time base only. It is not intended to drive external pins in the EnDat configuration.

### Single-sampling assumption

The current build uses `SAMPLING_METHOD = SINGLE_SAMPLING`, so EPWM1 INT, `Cla1Task1`, and `motor1ControlISR()` all run at `10 kHz`, while EPWM7 runs at `40 kHz`.
