  ...
  snapshot.telemetry_samples = limitList(
    [
      ...(snapshot.telemetry_samples || []),
      {
        ...
        offset_current_bs: Number(status?.offset_current_bs || 0),
        offset_current_cs: Number(status?.offset_current_cs || 0),
        fcl_latency_us: Number(status?.fcl_latency_us || 0),
        raw_position_offset_pu: Number(status?.raw_position_offset_pu || 0),
      },
    ],
    TELEMETRY_LIMIT,
  );
...
export function derivePrimaryMcuDetail(snapshot: SessionSnapshot | null): McuDetail | null {
  ...
  return {
    ...
    telemetry: {
      ...
      fcl_latency_us: Number(status?.fcl_latency_us ?? 0),
      raw_position_offset_pu: Number(status?.raw_position_offset_pu ?? 0),
      endat_crc_fail_count: Number(status?.endat_crc_fail_count ?? 0),
      vdc_bus: Number(status?.vdc_bus ?? 0),
